...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public int getID(int position){
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursorCourses = db.rawQuery("SELECT * FROM " + STUDENT_TABLE, null);
        cursorCourses.move(position);
        int ID = Integer.parseInt(cursorCourses.getString(0));
        return ID;
    }

    public int updateStudent(StudentModel STUDENTModel, int id){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(STUDENT_NAME,STUDENTModel.getName());
        cv.put(STUDENT_ROLL,STUDENTModel.getRollNmber());
        cv.put(STUDENT_ENROLL,STUDENTModel.isEnroll());
        return db.update(STUDENT_TABLE, cv, STUDENT_ID + "=?", new String[]{String.valueOf(id)});
    }
